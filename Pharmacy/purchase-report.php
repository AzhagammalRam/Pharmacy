...
function rptDisplay(){
  ...
  var d1 = $('#dtfrom').val(), d2 = $('#dtto').val(), pm = $('#paymentmode').val();
	...
	$.ajax({
        ...
        if(x[i].duedate=='00/00/0000')
        {
          x[i].duedate='-';
        }
				var tr = "<tr><td>"+x[i].date+"</td><<td><a href='printpurchase.php?billno="+x[i].billno+"' target='_blank' style='text-decoration:none;'>"+x[i].billno+"</a></td><td>"+x[i].supplier+"</td><td>"+x[i].type+"</td><td>"+x[i].pending+"</td><td>"+x[i].duedate+"</td><td class='tax1'>"+x[i].paidamt+"</td><td>"+x[i].paidto+"</td></tr>";
				 $(table).append(tr);
			}
