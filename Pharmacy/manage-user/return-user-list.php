<?php
	include("../config.php");
	$sql = "SELECT id, username, userid, password, role, image, status FROM tbl_users ORDER BY id desc";
	...
	while($rs = mysqli_fetch_array($res)){
		...
		$option1 = ($status == 1) ? '<div class="hidden-sm hidden-xs action-buttons">
									...
									<div class="hidden-sm hidden-xs action-buttons"> 
										<a id="view" title="View" class="blue" href="#" data-val="'.$rs['id'].'"> <i class="ace-icon fa fa-search-plus bigger-130"></i> </a> 
										<a id="enable" title="Edit" class="green" href="#" data-val="'.$rs['id'].'"> <i class="ace-icon fa fa-check bigger-130"></i> </a>
