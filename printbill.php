...
$pdf->MultiCell(8.6, 0.25, "ZAARA CLINIC" , '0', "C");
$pdf->Ln(.1);
$pdf->SetFont('Arial','',12);
//$pdf->MultiCell(8.6, 0.25, "(Unit of Murugan Hospitals)" , '0', "L");
//$pdf->SetFont('Times','I',10);
$pdf->MultiCell(8.6, 0.2, "No.114, Kilpauk Garden Road," , '0', "C");
$pdf->MultiCell(8.6, 0.2, "Kilpauk, Chennai-600 010." , '0', "C");
$pdf->MultiCell(8.6, 0.2, "Ph.No.8939511585/044-79672529" , '0', "C");
...
$pdf->Ln(.25);
$cmd = mysqli_query($db,"SELECT datentime, patientname, drname, billno, paidamt,phno,age FROM tbl_billing WHERE billno = '$billno' and del_status != 1");
...
if($type==1){
	...
	$pdf->myCell($w+1,$h-0.1,7.3,$amount);
	...
	$pdf->Cell(6.8, 0.25, 'Total Amount  ', '1','0', "R");
	if($type==1){
	...
	$pdf->Ln();
	if($r2['discount']!='0'){
	$pdf->Cell(6.8, 0.25, 'Discount Amount  ', '1','0', "R");
	$pdf->Cell(1.5, 0.25, $r2['discount']*$exchange_rate, '1','0', "C");
	$pdf->Ln();
	$pdf->Cell(6.8, 0.25, 'Total  ', '1','0', "R");
	if($type==1){
	$pdf->Cell(1.5, 0.25, $r2['totalamt']*$exchange_rate, '1','0', "C");
	}
	else if ($type==2) {
	...
	$pdf->Cell(6.8, 0.25, 'Payment Mode  ', '1','0', "R");
